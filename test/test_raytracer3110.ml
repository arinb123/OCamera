...
let scalar_mult_dist =
  Test.make ~name:"scalar multiplication distributes over vector addition"
    arb_vec (fun v1 ->
      let v2 =
        V.make
          ( Random.float 200. -. 100.,
...
let cross_orthogonal =
  Test.make ~name:"cross product of orthogonal vectors is nonzero" arb_vec
    (fun v1 ->
...
let length_dot =
  Test.make ~name:"length squared equals dot product with itself" arb_vec
